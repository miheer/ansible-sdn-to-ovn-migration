...
def main():
    ...
    while time.time() - start_time < timeout:
        available_cmd = "oc wait co --all --for='condition=AVAILABLE=True' --timeout=10s"
        progressing_cmd = "oc wait co --all --for='condition=PROGRESSING=False' --timeout=10s"
        degraded_cmd = "oc wait co --all --for='condition=DEGRADED=False' --timeout=10s"
