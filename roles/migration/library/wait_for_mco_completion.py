...
def wait_for_mco(timeout):
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            run_command("oc wait mcp --all --for='condition=UPDATED=True' --timeout=10s")
            run_command("oc wait mcp --all --for='condition=UPDATING=False' --timeout=10s")
            run_command("oc wait mcp --all --for='condition=DEGRADED=False' --timeout=10s")
