...
def run_command(cmd):
    try:
        ...
        raise Exception(f"Command '{cmd}' failed: {e.stderr.strip()}")


def get_machine_config_status():
    output = run_command("oc describe node | egrep 'hostname|machineconfig'")
    nodes = re.findall(
...
def verify_machine_config(config_name):
...
def main():
    ...
    try:
        nodes = get_machine_config_status()
        issues = []
        for node in nodes:
            if node["state"] != "Done":
            ...
            if not verify_machine_config(node["currentConfig"]):
                issues.append(
